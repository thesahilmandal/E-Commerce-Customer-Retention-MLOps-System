...
class InferenceValidation:
    ...
    def _is_type_compatible(self, expected: str, actual: str) -> bool:
        ...
        model robustness. Extracted backend-specific suffixes (like PyArrow) are normalized 
        for seamless compatibility across compute engines.
        """
        # Normalize types and strip PyArrow backend suffixes
        expected = str(expected).lower().replace("[pyarrow]", "")
        actual = str(actual).lower().replace("[pyarrow]", "")
